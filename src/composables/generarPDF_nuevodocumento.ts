...
export function pdf_documento() {
    const generarPDF =  async (documento: DocumentoFull) => { 
        ...
        NProgress.start();
        const textocodigo = "hola";//BASE_URL+"/seguimiento/"+documento.codigo.toString();
        ...
        doc.setDrawColor(0, 40, 133);
        doc.setLineWidth(1);
        doc.roundedRect(margin - 7, 150, pageWidth - margin * 2 + 14, 135, 2, 2); // Rectángulo como borde
        ...
        doc.text("Mesa de Partes", pageWidth / 2, PosicionY, { align: "center" });
        ...
        if(documento.administrado_tipodocumento==="RUC"){
            doc.setFont("helvetica", "bold");
            doc.text("- REMITENTE:", col1+4, PosicionY);
            doc.setFont("helvetica", "normal");
            PosicionY += addTextWithWrap(`${checkValue(documento.administrado_razonsocial)}`, col2, PosicionY, maxWidth);
            PosicionY = PosicionY-2;
        }else{
            doc.setFont("helvetica", "bold");
            doc.text("- REMITENTE:", col1, PosicionY);
            doc.setFont("helvetica", "normal");
            PosicionY += addTextWithWrap(checkValue(documento.administrado_nombre + ' ' + documento.administrado_apellidomat + ' ' +  documento.administrado_apellidopat), col2, PosicionY, maxWidth);
            PosicionY = PosicionY-2;
        }
        ...
        // ----------------------
        doc.setFont("helvetica", "bold");
        doc.text("- COMENTARIO:", col1, PosicionY);
        doc.setFont("helvetica", "normal");
        PosicionY += addTextWithWrap(checkValue(documento.descripcion), col2, PosicionY, maxWidth);
        PosicionY = PosicionY-2;
        ...
        doc.setFont("helvetica", "normal");
        PosicionY += addTextWithWrap(checkValue(documento.pase_nombre_destino), col2, PosicionY, maxWidth);
        PosicionY = PosicionY-2;
        ...
        doc.setFont("helvetica", "normal");
        doc.setFontSize(8);
        PosicionY += addTextWithWrap("* Para poder realizar seguimiento al documento registrado, debe activar su casilla electrónica en nuestro portal web.", col1, 135, maxWidth);
        PosicionY = PosicionY-2;
