          ...
          <DataTable
                ...
                <template #default="{ item, index, currentPage, itemsPerPage }">
                  <tr>
                    ...
                    <td  class="px-2 mx-0  ">                      
                      <div class="py-1 text-sm alert alert-warning mb-0 pb-0"  v-if="item.descripcion!==''" >
                          <i class="fa fa-comment"></i>    {{ item.descripcion }}
                        </div>
                    </td>
                    ...
                    <td>
                      ...
                      <router-link v-if="item.pase_estadopase!=='Recibido' && item.pase_estadopase!=='Tramitado' && (Usuario.rol_id===1 ||Usuario.rol_id===4)"
                          :to="{ name: 'trabajador_documento_proyectar_editar', params: { iddocumento: item.iddoc } }" class="btn btn-danger btn-sm rounded-3 bg-red-400 border-0 text-sm">
                                <i class="ph-duotone ph-pencil f-20"></i> Corregir
                      </router-link>
                      
                      <div @click="Enviar_corregido_proyectado(item.iddoc)" 
                              v-if="Usuario.rol_id===4" 
...
import { Aceptar_documento_proyectado, actualizar_documentosFirmado, ConfirmarEnvio_documento_proyectado, Enviar_a_proyectado, get_listadocumentos_Observados_nombreUsuario, subir_adjunto } from '../../../services/DocumentoServices';
  ...
  setup() {
    ...
    const CargarExpedientes = async () => {
        try {
          ...
          NProgress.start();
          const RespuestaJSON = await get_listadocumentos_Observados_nombreUsuario(Usuario.value.username);
    ...
    const headers = ref([
      ...
      { text: 'USUARIO.', width: '10%', key: 'usuario_nombre', type: 'string' as const, sortable: true, filterable: true },
      { text: 'ASUNTO', width: '30%', key: 'asunto', type: 'string' as const, sortable: false, filterable: false },
      { text: 'OBSERVACIÓN', width: '20%', key: 'estado', type: 'string' as const, sortable: true, filterable: false },
