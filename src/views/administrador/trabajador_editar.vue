...
import {  get_Trabajador, subir_foto, upd_TrabajadorActual, } from '../../services/trabajadorService';
  ...
  setup() {
    ...
    const cargarTrabajador = async (idtrabajador: number) => {
      ...
      try {
        ...
        if(RespuestaJSON.success){
          trabajadorForm.value= RespuestaJSON.data;
        }else{
          Notif.Error(RespuestaJSON.message);
        }
      } catch (error) {
    ...
    const guardarTrabajador = async () => {
      ...
      try {
        NProgress.start();
        const RespuestaJSON = await upd_TrabajadorActual(trabajadorForm.value);
        if (RespuestaJSON.success) {
