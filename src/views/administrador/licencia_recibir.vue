<template>
    ...
    <div class="row">
        <div class="col-md-9">
            <!-- ------------------------------ -->            
            <div class="card">
              <div class="card-header bg-secondary pb-1 pb-0">
                <h5 class="text-white">
                  <i class="fa fa-edit"></i> Datos de Solicitud
                </h5>
              </div>              
              <div class="card-body bg-light-blue-accent-3">
                ...
                <div class="row">
                  ...
                  <div  class="col-md-6">
                    <div class="mb-2 ">
                      ...
                      <input disabled v-model="licencia.fecharegistro" type="text" class="form-control"
        ...
        <div class="col-md-3">
                ...
                <div class="sticky-top mt-0 pt-0 text-center" style="top:74px">
                    ...
                    <div class="pb-3">
                        <img  class="img-fluid wid-100 " 
                          src="../../assets/images/document.png">
                    </div>
                    
                    <!-- ------------------------ -->
                    <div class="col-12">
                        <button  class="btn btn-primary d-inline-flex btn-lg  "
                                @click="guardarLicencia">
                                <i class="fas fa-save p-r-5"></i>Guardar Cambios</button>
                        <!-- -------------------------------- -->
                        <button @click="Aceptar_solicitud" class="btn btn-success mt-2 d-inline-flex btn-lg">
                            <i class="fas fa-check p-r-5"></i>Aceptar Solicitud </button>
                        <!-- -------------------------------- -->
                        <button  class="btn btn-danger d-inline-flex btn-lg mt-2 "
                                @click="openModal">
                                <i class="fas fa-times p-r-5"></i>Rechazar Solicitud</button>
                        <!-- -------------------------------- -->
                        <router-link :to="{ name: 'ListarSolicitudesLicencia'}" 
                                class="btn btn-lg d-inline-flex btn-secondary mt-2">
                           
                                <i class="fas fa-arrow-left p-r-5"></i>Cancelar
                        </router-link>
                    </div>
                </div>
                <!-- ------------------------------ -->
  ...
  import { get_licencia, aceptar_solicitud_licencia, rechazar_solicitud_licencia, upd_licenciaActual} from '../../services/LicenciaService';
...
import { FormatFecha } from '../../utils/FormatFecha';
  ...
  export default defineComponent({
    ...
    setup() {
     ...
     const Aceptar_solicitud = async () => {
        ...
        if (isConfirmado) {
          NProgress.start();
          try {
              const response = await aceptar_solicitud_licencia(idlicencia.toString());
              if(response.success){
                Notif.Success("El registro fue aceptado.");
    ...
    const closeModal = () => {
      ...
      onMounted(async () => {
        await FunList.load_TramitesTupa_Oficina(usuario.value.oficina_id);
        cargarLicencia();
      });
