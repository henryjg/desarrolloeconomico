<template>
  <div class="w-90 m-auto">
    <div class="row pt-4 pb-3 border-top">
      <div class="col-lg-4 ">
        <div class="seccion-titulo f-14 pb-4 ">
              <h2>Mesa de Partes Virtual</h2>
              <hr class="mb-4 my-1 ">
              <p>Estimado(a) ciudadano(a), Con la finalidad de llevar un buen control de plazos de los documentados presentados, se establece lo siguiente:</p>
              <h5>Horario:</h5>
              <ul>
              ...
              <h5>El documento:</h5>
              <ul>
                  <li>Todo documento debe estar debidamente foliado.</li>
                  <li>El documento adjunto debe ser claro y legible.</li>
              </ul>
              <hr class="mb-4 my-1 ">
              <h4>Instructivo:</h4>
              <VideoFondo/>
              ...
              <router-link :to="{ name: 'Seguimiento'}" >
                  <div class="btn btn-success btn-lg">
                    <i class="fa fa-search p-r-10 "></i>Consultar Expediente</div>
              </router-link>
    ...
    <!-- {{ documento }} -->
            ...
            <div  v-if="secuencia !== 3" class="row pb-1" >
                      ...
                      <div class="mb-1">
                          <div class="row g-1">
                            ...
                            <div class="col-md-1 align-content-end">
                              ...
                              <div class="btn btn-secondary btn-sm rounded-1 w-100" @click="Buscar_Administrado_x_DNI">
                  ...
                  <div class="row pt-2">
                    <div class="col-md-12">
                      <table v-if="AdministradoExistente.numdocumento!==''"  class="table table-sm mb-1" id="pc-dt-simple">
                        <thead>
                          <tr >
                            ...
                            <th style="width:50%; font-size: 11px;" class="py-1 b-light f-w-600">Nombres y Apellidos</th>
                            <th style="width:10%; font-size: 11px;" class="py-1 b-light f-w-600">Celular</th>
                            <th style="width:25%; font-size: 11px;" class="py-1 b-light f-w-600">Correo</th>
                            <th style="width:5%; font-size: 11px;" class="py-1 b-light"></th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr >
                            <td class="py-1 text-wrap">{{ AdministradoExistente.tipodocumento }} - {{ AdministradoExistente.numdocumento }}</td>
                            <td v-if="AdministradoExistente.tipodocumento==='DNI'" class="py-1 text-wrap">{{ AdministradoExistente.nombre }} {{ AdministradoExistente.apellidopat }} {{ AdministradoExistente.apellidomat }}</td>
                            <td v-else class="py-1 text-wrap">{{ AdministradoExistente.razonsocial }}</td>
                            <td class="py-1 text-wrap">{{ AdministradoExistente.celular }}</td>
                            <td class="py-1 text-wrap">{{ AdministradoExistente.correo }} </td>
                      ...
                      <div v-else class="alert alert-warning">
                        <i class="ph-duotone ph-warning align-content-center"></i> Cargue o registre sus datos en la opción Nuevo</div>
                    </div>
                  </div>
                </div>
                <!-- -------------------------------------------- -->
            ...
            <div v-if="secuencia !== 3" class="row">
              <div class="d-flex" style="z-index: 2">
                ...
                <div class="row g-1">
                  <div class="col-md-12">
                      <div class="row g-1">
                        ...
                        <div class="col-md-5">
                            <label class="form-label f-13">Numeración: </label>
                            <input maxlength="225" type="text" v-model="documento.cabecera" placeholder="Ejm: 0025-2025-HJG" class="form-control" >
                      ...
                      <div class="d-flex">
                        <multiselect v-model="DestinoOficina"
                                    @update:modelValue="on_changeOficina"
                                    class="f-6"
                                    label="destino"
                                    track-by="id"
                                    placeholder="Seleccionar"
                  ...
                  <div class="col-md-12">
                    ...
                    <div class="px-0 rounded-1 mt-1">
                      <div v-if="Array_archivos.length===0" @click="openModal_Adjunto"
                          ...
                          <p class="m-auto">Agregar documento</p>
                      </div> 
                      ...
                      <ErrorMessage :error="errorsdocumento.pdf_principal" />
                    </div>
                    <!-- -*-*-*-*-*-*-*-*- -->
              ...
              <!-- -------------------------------------------- -->
                ...
                <div v-if="StoreList.ListaRequisitosAsignados.length>0 && documento.estupa">
                  <label class="form-label f-13 pb-0 mb-0">Requisitos Obligatorios:</label>
                ...
                <div v-if="secuencia !== 3"  class="col-lg-12 text-center " >
                  <div
                      ...
                      ></div>
                  <hr>
                  <button @click="RegistrarDocumento" class="btn btn-lg  btn-danger mt-3 m-auto">
                      <i class="fas fa-user-plus p-r-5"></i>Registrar Trámite</button>
                </div>
                <!-- -------------------------------------------- -->
              </div>
            </div>
              ...
              <div class="col-lg-12 card f-16 text-inter text-center mb-2 p-1 b-success rounded-2">
                  <div class="card-body p-5">
                    ...
                    <div class="col-lg-12 py-5">
                        <i class="fa fa-check-circle f-50 pb-3 text-success"></i>
                        <p class="text-success f-20">Registro Completado Satisfactoriamente</p>
                        <br>
                        <h4> So código de Expediente es:</h4>
                        <h2> 000{{DocumentoRegistrado.numerodocumento}} - {{ DocumentoRegistrado.anio }}</h2>
                        <button @click="generarPDF(DocumentoRegistrado)"  class="btn btn-warning bg-yellow-700 btn-lg mx-1">
                          <i class="ph-duotone ph-printer px-1"></i>Imprimir Constancia</button>
                    </div>
                  ...
                  <div class="col-md-12">
                      <div class="mb-1">
                        <div class="row g-1">
                          ...
                          <div class="col-md-4">
                            <label class="form-label mb-0 f-13">Número de documento</label>
                            <input maxlength="11" type="text" v-model="persona.numdocumento" @change="on_numDocChange" class="form-control" placeholder="">
                          </div>
                        </div>
                      </div>
                      <!-- ---------------------------------------- -->
...
import { Destino, Destino_Oficina, initializeUsuarioLog, Oficina, PersonaRegistro, Tramite, UsuarioLog } from '../../types/interfaces';
...
import { add_documento_mesapartes_virtual, subir_adjunto } from '../../services/DocumentoServices';
...
import { Adjunto, DocumentoExterno, DocumentoFull,  initialize_Adjunto,  initialize_CrearDocumentoExterno_virtual, initialize_CrearDocumentoExternoErrores_virtual, initialize_DocumentoFull, initialize_tipodocumento, tipodocumento } from '../../interfaces/documento.interface';
...
import { get_destinos_general, get_destinos_unidadesorganicas } from '../../services/DestinosServices';
...
export default {
    ...
    setup () {
        ...
        const siteKey = '6LckhAcrAAAAAE_KqANeGFKosCO_rVyzT5KavRQQ'; // Reemplaza con tu Site Key
        ...
        const DestinoOficina = ref<Destino_Oficina>();
        ...
        onMounted(async () => {
          ...
          loadRecaptchaScript()
            ...
            .catch((error) => {
              ...
              Notif.Error('No se pudo cargar reCAPTCHA. Intenta recargar la página.');
            });
        });
        ...
        const onRecaptchaVerified = (token: string) => {
          recaptchaToken.value = token;
          console.log("Token: " + recaptchaToken.value);
        };

        // Callback cuando el reCAPTCHA expira -----------------------------------------
        const onRecaptchaExpired = () => {
          recaptchaToken.value = null;
          console.log("Expire: " + recaptchaToken.value);
        };
        ...
        const SubirArchivo = async () => {
            ...
            ArchivoTemporal.value.nombrefile='pdf_documentoprincipal';
            ArchivoTemporal.value.tipo_archivo='pdf_documentoprincipal';
            ...
            try {
              ...
              if (RespuestaJSON.success) {
                  ...
                  if(ArchivoTemporal.value.nombrefile==='pdf_documentoprincipal'){
                    documento.value.pdf_principal = HOST_URL+RespuestaJSON.data.toString();
                  }else if(ArchivoTemporal.value.nombrefile==='pdf_anexo1'){
                    documento.value.pdf_anexo1 = HOST_URL+RespuestaJSON.data.toString();
                  }else{
                    documento.value.pdf_anexo2 = HOST_URL+RespuestaJSON.data.toString();
                  }
                  // ------------------------------------------------------
                  closeModal_Adjunto();
              } else {
        ...
        // REGISTRAR DOCUMENTO  -------------------------------------------------------------
        // -----------------------------------------------
        const documento = ref<DocumentoExterno>( initialize_CrearDocumentoExterno_virtual());
        const errorsdocumento = ref<any>(initialize_CrearDocumentoExternoErrores_virtual());
        ...
        watch(() => documento.value.pdf_principal, (newVal) => validateField('pdf_principal', newVal));
        watch(() => documento.value.cabecera, (newVal) => validateField('cabecera', newVal));
        ...
        const RegistrarDocumento = async () => {     
          ...
          if(documento.value.administrado_id===0){
            Alerta.Error("Solicitante Incompleto","Ingresar los datos del solicitante");
            return
          }else if(!validateDocumento()) {
            Alerta.Error("Datos Incompletos","Ingrese todos los campos obligatorios");
          ...
          try {
            // Verifica si el reCAPTCHA ha sido completado correctamente
              if (!recaptchaToken.value) {
                Alerta.Error("Validación Incompleta", "Por favor, completa el reCAPTCHA");
                return;
              }else{
                 // Agrega el token de reCAPTCHA al objeto documento
                // documento.value.recaptcha_token = token;

                NProgress.start();
                //--------
                const RespuestaJSON = await add_documento_mesapartes_virtual(documento.value, recaptchaToken.value);
                if (RespuestaJSON.success) {
        ...
        const reset_formularioDatos = () =>{
          persona.value= initializeAdministrado();
          documento.value = initialize_CrearDocumentoExterno_virtual();
          AdministradoExistente.value= initializeAdministrado();
          secuencia.value = 1;
          errors.value = initializeAdministrado();
          errorsdocumento.value = initialize_CrearDocumentoExternoErrores_virtual();
          // Reset Form
          DestinoOficina.value = {
              id: '',       //id buzon
              destino: '',  //nombre buzon
              sigla: '',
              oficina: '',
              destino_usuario: '',
              rol_nombre: '',
